...
2. **D2 - identity và Professional access (đã xây dựng):** User/password hash, JWT access token, rotating refresh session, `SalonMembership` OWNER authorizer, `AdminAccess` và `ProfessionalProfile` độc lập. Register intent `PROFESSIONAL` tạo profile `PENDING`; self-service `GET/PATCH /professionals/me` cho dữ liệu cơ bản đã có. Hold/confirm/booking chỉ nhận profile `ACTIVE`; Owner/Admin không kế thừa quyền này. DB đã có foundation additive cho verification case, credential, private document, quyền Admin review và password-reset token ở cấp User; chưa bật verification enforcement.
...
9. **Professional trust follow-up (workflow còn lại):** basic self-service onboarding đã có; private upload/finalize, manual Admin review, password-reset request/confirm và email delivery chưa có. Chỉ sau backfill/UAT mới dùng verification/credential expiry để chặn hold/confirm mới.
