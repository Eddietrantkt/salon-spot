...
export function AdminConsolePage({ initialSession, onSignIn, onSessionRestored, onSessionEnded }: AdminConsolePageProps): JSX.Element {
  ...
  return <main className="page-shell admin-shell">
    <header><p className="eyebrow">{t('THE SALON SPOT · OPERATIONS', 'THE SALON SPOT · VẬN HÀNH')}</p><h1>{auth ? t(`Operations for ${auth.user.displayName}`, `Không gian quản trị của ${auth.user.displayName}`) : t('System administration', 'Quản trị hệ thống')}</h1><p className="lead">{t('Monitor operations and manage accounts with controlled access. Admins do not own or manage salons on behalf of owners.', 'Theo dõi vận hành và quản lý tài khoản với quyền truy cập được kiểm soát. Quản trị viên không sở hữu hoặc quản lý salon thay cho chủ salon.')}</p>{auth && <span className="portal-identity">{t('Admin account', 'Tài khoản Quản trị')} · {auth.user.email}</span>}</header>
