...
export function SearchWorkspacesForm({
  ...
  return (
    <form className={`search-form search-form-${variant}${isLoading ? ' search-form-loading' : ''}`} onSubmit={onSubmit} aria-busy={isLoading}>
      ...
      <button className="search-submit" type="submit" disabled={isLoading}><span>{isLoading ? t('Searching…', 'Đang tìm…') : t('Search workspaces', 'Tìm không gian')}</span><span className="search-submit-icon" aria-hidden="true">→</span></button>
