...
export function OwnerConsolePage({ initialSession, onSignIn, onSessionRestored, onSessionEnded }: OwnerConsolePageProps): JSX.Element {
  ...
  return (
    <main className="page-shell owner-shell">
      <header><p className="eyebrow">{t('THE SALON SPOT · OWNER CONSOLE', 'THE SALON SPOT · KHU VỰC CHỦ SALON')}</p><h1>{auth ? t(`${auth.user.displayName}'s salon spaces`, `Không gian salon của ${auth.user.displayName}`) : t('Manage your salon spaces', 'Quản lý không gian salon')}</h1><p className="lead">{t('Create salons, workspaces, and rental options. Management permissions are always limited to the relevant salon.', 'Tạo salon, không gian và gói thuê. Quyền quản lý luôn chỉ giới hạn trong salon tương ứng.')}</p>{auth && <span className="portal-identity">{t('Owner account', 'Tài khoản Chủ salon')} · {auth.user.email}</span>}</header>
