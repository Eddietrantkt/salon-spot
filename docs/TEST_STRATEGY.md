...
| URL navigation and session boundary | Route state covers discovery query, Workspace detail, Bookings, Owner, Admin, Professional onboarding and login return path; packaged Chromium gate checks deep-link/refresh/back, Owner/Admin session boundary and 320/375/768 navigation | Typecheck + `pnpm p1:browser` disposable runtime |
...
Local MySQL E2E previously verified refresh-cookie rotation, logout revocation, idempotent Workspace retry, validation errors, fresh migration on MySQL 8.4, and concurrent D5/open-block plus D6-D8 booking races. The dated 28/08 result (API 39 suite/99 test; web 7/7) is historical only. Current release evidence must be reproduced with `pnpm p1:mysql` and `pnpm p1:browser`: the latter creates a packaged Compose fixture with random project/ports/volumes, seeds demo roles, then tears it down. It does not replace authenticated API/MySQL mutation proof or the manual timezone matrix. Automated tests still do not prove every database transaction race.
...
Chromium Playwright runs through `pnpm p1:browser`, which creates and disposes its own same-origin Compose runtime; traces, video and screenshots are retained on failure. Its checked-in flows cover deep links, refresh/back, role-specific login boundaries and 320/375/768 navigation. It does not replace the manual two-Salon/two-browser-timezone and mutation UAT record; attach request IDs and direct database results to `P1_TEST_EVIDENCE.md`.
