...
```powershell
pnpm runtime:up
docker compose --env-file .env.runtime -f compose.runtime.yaml ps
```
...
docker compose --env-file .env.runtime -f compose.runtime.yaml exec worker node -e "fetch('http://127.0.0.1:3001/worker/health/ready').then(r => { console.log(r.status); process.exit(r.ok ? 0 : 1) })"
```
...
1. Inspect the structured worker logs: `docker compose --env-file .env.runtime -f compose.runtime.yaml logs --tail=200 worker`.
...
For a controlled crash rehearsal, run `docker compose --env-file .env.runtime -f compose.runtime.yaml kill worker`, then verify a new worker container start time, fresh heartbeats, and eventual delivery of an expired-lease fixture. The existing local notification sink and media cleanup transition outbox state using conditional leases. This proves one database delivery transition for the fixture; it does **not** prove exactly-once delivery to a future external provider across a crash after that provider accepts a request.
...
3. Create or retain a valid `PROCESSING` outbox event whose `availableAt` lease is expired, then kill the worker container.
4. Confirm Compose restarts it automatically, all three heartbeat timestamps advance, and the fixture becomes `DELIVERED` with one successful conditional claim.
